...
/** Generates default traits for a B+ tree used as a set. It estimates leaf
 * and
 * inner node sizes by assuming a cache line size of 256 bytes. */
template <typename _Key> class btree_default_set_traits {
public:
  ...
  static const int innerslots =
      BTREE_MAX(8, 256 / (sizeof(_Key) + sizeof(void *)));
...
/** Generates default traits for a B+ tree used as a map. It estimates leaf
 * and
 * inner node sizes by assuming a cache line size of 256 bytes. */
template <typename _Key, typename _Data> class btree_default_map_traits {
public:
  /// If true, the tree will self verify it's invariants after each insert()
  /// or erase(). The header must have been compiled with BTREE_DEBUG defined.
  static const bool selfverify = false;
  ...
  static const int leafslots =
      BTREE_MAX(8, 256 / (sizeof(_Key) + sizeof(_Data)));
 ...
 * implementation in Cormen's Introduction into Algorithms, Jan Jannink's
 * paper
 ...
 * fulfilled in theory, however in practice this B+ tree performs better than
 * a
 * red-black tree by using more memory. The insertion function splits the
 * nodes
...
template <typename _Key, typename _Data,
          ...
          bool _Duplicates = false, typename _Alloc = std::allocator<_Value>,
...
private:
  ...
  struct inner_node : public node {
    ...
    inline inner_node(const unsigned short l) : node(l) {}

    /// Construction during restore from node top
    inline inner_node(const node &top) : node(top.level, top.slotuse) {}
  ...
  struct leaf_node : public node {
    ...
    /// Set variables to initial values
    inline leaf_node() : node(0), prevleaf(NULL), nextleaf(NULL) {}
...
private:
  ...
  struct btree_pair_to_value {
    ...
    inline value_type operator()(pair_type &p) const { return p.first; }
    /// Convert a fake pair type to just the first component
    inline value_type operator()(const pair_type &p) const { return p.first; }
  };
  ...
  struct btree_pair_to_value<value_type, value_type> {
    ...
    inline value_type operator()(pair_type &p) const { return p; }
    /// Identity "convert" a real pair type to just the first component
    inline value_type operator()(const pair_type &p) const { return p; }
  };
...
public:
  ...
  public:
    ...
    inline const key_type &key() const { return currnode->slotkey[currslot]; }
  ...
  public:
    ...
    inline const_iterator() : currnode(NULL), currslot(0) {}
  ...
  public:
    ...
    inline reverse_iterator() : currnode(NULL), currslot(0) {}
  ...
  public:
    ...
    inline const_reverse_iterator() : currnode(NULL), currslot(0) {}
...
public:
  ...
  struct tree_stats {
    ...
    /// Zero initialized
    inline tree_stats() : itemcount(0), leaves(0), innernodes(0) {}

    /// Return the total number of nodes
    inline size_type nodes() const { return innernodes + leaves; }
...
public:
  ...
  explicit inline btree(const key_compare &kcf,
      ...
      : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL), m_key_less(kcf),
        m_allocator(alloc) {}
...
public:
  ...
  protected:
    ...
    inline explicit value_compare(key_compare kc) : key_comp(kc) {}
  ...
  inline key_compare key_comp() const { return m_key_less; }

  /// Constant access to a constructed value_type comparison object. Required
  /// by the STL
  inline value_compare value_comp() const { return value_compare(m_key_less); }
...
private:
  ...
  inline inner_node *allocate_inner(unsigned short level) {
    inner_node *n = new (inner_node_allocator().allocate(1)) inner_node(level);
    m_stats.innernodes++;
    return n;
  }
...
public:
  ...
  inline iterator begin() { return iterator(m_headleaf, 0); }
  ...
  inline const_iterator begin() const { return const_iterator(m_headleaf, 0); }
...
private:
  ...
  inline int find_lower(const node_type *n, const key_type &key) const {
    if (0 && sizeof(n->slotkey) > traits::binsearch_threshold) {
      ...
      BTREE_PRINT("btree::find_lower: on " << n << " key " << key << " -> "
                                           << lo << " / " << hi);
  ...
  inline int find_upper(const node_type *n, const key_type &key) const {
    if (0 && sizeof(n->slotkey) > traits::binsearch_threshold) {
      ...
      BTREE_PRINT("btree::find_upper: on " << n << " key " << key << " -> "
                                           << lo << " / " << hi);
...
public:
  ...
  inline size_type size() const { return m_stats.itemcount; }

  /// Returns true if there is at least one key/data pair in the B+ tree
  inline bool empty() const { return (size() == size_type(0)); }
  ...
  inline const struct tree_stats &get_stats() const { return m_stats; }
...
public:
  ...
  inline bool operator>(const self_type &other) const { return other < *this; }
...
public:
  ...
  inline btree(const btree &other)
      : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL),
        m_stats(other.m_stats), m_key_less(other.key_comp()),
        m_allocator(other.get_allocator()) {
    if (size() > 0) {
      m_stats.leaves = m_stats.innernodes = 0;
      if (other.m_root) {
  ...
  */
  std::pair<iterator, bool> insert_descend(node *n, const key_type &key,
    ...
    if (!n->isleafnode()) {
      ...
      if (newchild) {
        BTREE_PRINT("btree::insert_descend newchild with key "
                    << newkey << " node " << newchild << " at slot " << slot);
  ...
  void split_inner_node(inner_node *inner, key_type *_newkey, node **_newinner,
    ...
    BTREE_PRINT("btree::split_inner_node on "
...
public:
  ...
  template <typename Iterator> void bulk_load(Iterator ibegin, Iterator iend) {
    ...
    BTREE_PRINT("btree::bulk_load, level 0: "
                << m_stats.itemcount << " items into " << num_leaves
                << " leaves with up to "
    ...
    size_t num_parents =
        (num_leaves + (innerslotmax + 1) - 1) / (innerslotmax + 1);

    BTREE_PRINT("btree::bulk_load, level 1: "
    ...
    for (size_t i = 0; i < num_parents; ++i) {
      ...
      --n->slotuse; // this counts keys, but an inner node has keys+1
                    // children.
    ...
    for (int level = 2; num_parents != 1; ++level) {
      ...
      num_parents =
          (num_children + (innerslotmax + 1) - 1) / (innerslotmax + 1);

      BTREE_PRINT("btree::bulk_load, level "
...
private:
  ...
  struct result_t {
    ...
    inline bool has(result_flags_t f) const { return (flags & f) != 0; }
...
public:
  ...
  bool erase_one(const key_type &key) {
    ...
    result_t result =
        erase_one_descend(key, m_root, NULL, NULL, NULL, NULL, NULL, 0);
  ...
  void erase(iterator iter) {
    BTREE_PRINT("btree::erase_iter(" << iter.currnode << "," << iter.currslot
                                     << ") on btree size " << size());
    ...
    result_t result =
        erase_iter_descend(iter, m_root, NULL, NULL, NULL, NULL, NULL, 0);
...
private:
  ...
  result_t erase_one_descend(const key_type &key, node *curr, node *left,
    ...
    } else // !curr->isleafnode()
    {
      ...
      if (result.has(btree_update_lastkey)) {
        if (parent && parentslot < parent->slotuse) {
          BTREE_PRINT("Fixing lastkeyupdate: key "
  ...
  result_t erase_iter_descend(const iterator &iter, node *curr, node *left,
    ...
    if (curr->isleafnode()) {
      ...
      if (iter.currslot >= leaf->slotuse) {
        BTREE_PRINT("Could not find iterator ("
                    << iter.currnode << "," << iter.currslot
    ...
    } else // !curr->isleafnode()
    {
      ...
      while (slot <= inner->slotuse) {
        ...
        if (slot == 0) {
          myleft = (left == NULL) ? NULL : (static_cast<inner_node *>(left))
  ...
  static result_t shift_left_leaf(leaf_node *left, leaf_node *right,
    ...
    BTREE_PRINT("Shifting (leaf) " << shiftnum << " entries to left " << left
                                   << " from right " << right
                                   << " with common parent " << parent << ".");
  ...
  static void shift_left_inner(inner_node *left, inner_node *right,
    ...
    BTREE_PRINT("Shifting (inner) " << shiftnum << " entries to left " << left
                                    << " from right " << right
                                    << " with common parent " << parent << ".");
    ...
    // copy the parent's decision slotkey and childid to the first new key on
    // the left
    left->slotkey[left->slotuse] = parent->slotkey[parentslot];
    left->slotuse++;

    // copy the other items from the right node to the last slots in the left
    // node.
  ...
  static void shift_right_leaf(leaf_node *left, leaf_node *right,
    ...
    BTREE_PRINT("Shifting (leaf) " << shiftnum << " entries to right " << right
                                   << " from left " << left
                                   << " with common parent " << parent << ".");
...
private:
  ...
  static void print_node(std::ostream &os, const node *node,
    ...
    if (node->isleafnode()) {
      ...
      for (unsigned int i = 0; i < depth; i++)
        os << "  ";
      os << "  leaf prev " << leafnode->prevleaf << " next "
         << leafnode->nextleaf << std::endl;
    ...
    } else // !n->isleafnode()
    {
      ...
      for (unsigned short slot = 0; slot <= inner->slotuse; ++slot) {
        ...
        if (inner->level == 2 && slot < inner->slotuse) {
          ...
          const inner_node *parenta =
              static_cast<const inner_node *>(inner->childid[slot]);
          const inner_node *parentb =
              static_cast<const inner_node *>(inner->childid[slot + 1]);

          const leaf_node *leafa = static_cast<const leaf_node *>(
              parenta->childid[parenta->slotuse]);
          const leaf_node *leafb =
              static_cast<const leaf_node *>(parentb->childid[0]);
  ...
  struct dump_header {
    ...
    inline bool same(const struct dump_header &o) const {
      return (signature[0] == 's' && signature[1] == 't' &&
              ...
              signature[8] == 'e' && signature[9] == 0 && signature[10] == 0 &&
              signature[11] == 0) &&
             (version == o.version) && (key_type_size == o.key_type_size) &&
             (data_type_size == o.data_type_size) &&
             (leafslots == o.leafslots) && (innerslots == o.innerslots) &&
...
private:
  ...
  node *restore_node(std::istream &is) {
    ...
    if (top.isleafnode()) {
      ...
      is.read(reinterpret_cast<char *>(&leaf) + sizeof(top),
              sizeof(leaf) - sizeof(top));
      if (!is.good())
        return NULL;

      leaf_node *newleaf = allocate_leaf();

      // copy over all data, the leaf nodes contain only their double linked
      // list pointers
      *newleaf = leaf;
      ...
      is.read(reinterpret_cast<char *>(&inner) + sizeof(top),
              sizeof(inner) - sizeof(top));
      if (!is.good())
        return NULL;

      inner_node *newinner = allocate_inner(0);

      // copy over all data, the inner nodes contain only pointers to their
      // children
      *newinner = inner;
