...
/** @brief Specialized B+ tree template class implementing STL's set
 * container.
 *
 ...
 * specifying an empty struct as data_type. All function are adapted to
 * provide
...
public:
  ...
  typedef stx::btree<key_type, data_type, value_type, key_compare, traits,
                     false, allocator_type, true>
      btree_impl;
...
public:
  ...
  void swap(self_type &from) { std::swap(tree, from.tree); }

public:
  // *** Key and Value Comparison Function Objects

  /// Constant access to the key comparison object sorting the B+ tree
  inline key_compare key_comp() const { return tree.key_comp(); }

  /// Constant access to a constructed value_type comparison object. required
  /// by the STL
  inline value_compare value_comp() const { return tree.value_comp(); }
...
public:
  ...
  inline size_type max_size() const { return tree.max_size(); }

  /// Return a const reference to the current statistics.
  inline const tree_stats &get_stats() const { return tree.get_stats(); }
...
public:
  ...
  bool exists(const key_type &key) const { return tree.exists(key); }
  ...
  /// either 0 or 1.
  size_type count(const key_type &key) const { return tree.count(key); }
...
public:
  ...
  /// copy of all keys.
  inline btree_set(const btree_set &other) : tree(other.tree) {}
...
public:
  ...
  /// difference to erase().
  bool erase_one(const key_type &key) { return tree.erase_one(key); }

  /// Erases all the key/data pairs associated with the given key.
  size_type erase(const key_type &key) { return tree.erase(key); }

  /// Erase the key/data pair referenced by the iterator.
  void erase(iterator iter) { return tree.erase(iter); }
...
public:
  ...
  void print(std::ostream &os) const { tree.print(os); }

  /// Print out only the leaves via the double linked list.
  void print_leaves(std::ostream &os) const { tree.print_leaves(os); }
...
public:
  ...
  void dump(std::ostream &os) const { tree.dump(os); }
  ...
  bool restore(std::istream &is) { return tree.restore(is); }
};
