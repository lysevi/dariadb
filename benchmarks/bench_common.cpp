...
void print(const std::vector<BenchmarkSummaryInfo::SpeedMetric> &metrics) {
  using namespace std;
...
dariadb::Id dariadb_bench::get_id_from(BenchmarkParams params, dariadb::Id id) {
  return (id + 1) * params.id_per_thread() - params.id_per_thread();
}

dariadb::Id dariadb_bench::get_id_to(BenchmarkParams params, dariadb::Id id) {
  return (id + 1) * params.id_per_thread();
}

BenchmarkSummaryInfo::SpeedMetric metrics(const std::string &name,
                                          const std::list<double> &values_list) {
  ...
  BenchmarkSummaryInfo::SpeedMetric result;
  result.name = name;
  for (auto v : values) {
    dariadb::Meas m;
    m.value = v;
...
void BenchmarkSummaryInfo::print() {
  ...
  std::cout << "writed: " << writed << std::endl;
  {
    ...
    SpeedMetric read_interval_sm = metrics("read interval", read_interval_speed_metrics);
...
void BenchmarkLogger::message(dariadb::utils::LOG_MESSAGE_KIND kind,
...
void dariadb_bench::readBenchmark(BenchmarkSummaryInfo *summary_info,
...
void dariadb_bench::thread_writer_rnd_stor(BenchmarkParams params, dariadb::Id id,
