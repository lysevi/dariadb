...
if [ "$CLANG" == "FALSE" ]; then
    ...
    if [[ "$GCOV" == "TRUE" ]]; then
       ...
       cmake -DBoost_USE_STATIC_LIBS=ON -DDARIADB_ENABLE_DOUBLECHECKS=ON -DBoost_USE_MULTITHREADED=ON  -DBoost_USE_STATIC_RUNTIME=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS_RELEASE="${CMAKE_CXX_FLAGS_RELEASE} --coverage -O0" \
		...
		if [[ -z "${SANITIZER}" ]]; then
		  if [[ "${IS_RELEASE}" == "TRUE" ]]; then	
			 ...
			 cmake -DBoost_USE_STATIC_LIBS=ON -DDARIADB_ENABLE_DOUBLECHECKS=OFF -DBoost_USE_MULTITHREADED=ON  -DBoost_USE_STATIC_RUNTIME=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS_RELEASE="${CMAKE_CXX_FLAGS_RELEASE}" \
			 -DCMAKE_EXE_LINKER_FLAGS="${CMAKE_EXE_LINKER_FLAGS}"  .
                  fi

		  if [[ -z "${IS_RELEASE}" ]]; then	
		   	 echo "default build."
			 cmake -DBoost_USE_STATIC_LIBS=ON -DDARIADB_ENABLE_DOUBLECHECKS=ON -DBoost_USE_MULTITHREADED=ON  -DBoost_USE_STATIC_RUNTIME=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS_RELEASE="${CMAKE_CXX_FLAGS_RELEASE}" \
			 -DCMAKE_EXE_LINKER_FLAGS="${CMAKE_EXE_LINKER_FLAGS}"  .
                  fi
		fi
	fi
