...
BOOST_AUTO_TEST_CASE(ReadInterval) {
  ...
  dariadb::Meas m;
  {
    ...
    }
    {
      ...
      BOOST_CHECK_EQUAL(output_in_point.size(),
                        size_t(2 + 3)); //+ timepoimt(3) with no_data
    ...
    BOOST_CHECK_EQUAL(output.size(),
                      size_t(5 + 3)); //+ timepoimt(3) with no_data
  }
  // from this point read not from firsts.
  {
    ...
    ds->append(m);
    {
      ...
      BOOST_CHECK_EQUAL(output_in_point.size(),
                        size_t(5 + 1)); //+ timepoimt(8) with no_data
      for (auto v : output_in_point) {
