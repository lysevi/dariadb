...
namespace dariadb {
namespace utils {
template <class Key, class Value, class EqualPred = std::equal_to<Key>> class LRU {
public:
  using pair_t = std::pair<Key, Value>;

  LRU(const size_t max_size) : _max_size(max_size) {}

  void set_max_size(const size_t max_size) { _max_size = max_size; }
  ...
  bool find(const Key &k, Value *out) {
    auto fres = std::find_if(_deq.begin(), _deq.end(),
                             [&k](const pair_t &p) { return EqualPred()(p.first, k); });
    if (fres == _deq.end()) {
      return false;
    } else {
      *out = (*fres).second;
