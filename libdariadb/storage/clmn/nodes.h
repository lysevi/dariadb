...
struct NodeHeader {
  ...
  Id meas_id;
  NodeHeader(gnrt_t g, node_sz_t sz, node_kind_t k, Id m_id) {
    meas_id = Id(0);
    gen = g;
    size = sz;
    kind = k;
...
struct Footer {
  ...
  Footer(Id m_id, node_addr_t p)
      : hdr(0, node_sz_t(1), NODE_KIND_FOOTER, m_id) {
...
struct Node {
  ...
  Node(gnrt_t g, node_sz_t sz, node_kind_t _k)
      : hdr(g, sz, _k, MAX_ID), stat() {
  ...
  static Ptr make_node(gnrt_t g, node_sz_t sz) {
    return std::make_shared<Node>(g, sz, NODE_KIND_NODE);
  }

  static Ptr make_root(gnrt_t g, node_sz_t sz) {
    return std::make_shared<Node>(g, sz, NODE_KIND_ROOT);
  }
...
struct Leaf {
  ...
  Leaf(gnrt_t g, node_sz_t sz) : hdr(g, sz, NODE_KIND_LEAF, MAX_ID) {
  ...
  static Ptr make_leaf(gnrt_t g, node_sz_t sz) {
    return std::make_shared<Leaf>(g, sz);
  }
};
...
public:
  ...
  virtual node_addr_t write_leaf(const Leaf::Ptr &leaf) = 0;
};
...
public:
  ...
  EXPORT virtual node_addr_t write_leaf(const Leaf::Ptr &leaf) override;
