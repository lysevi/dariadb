...
public:
  Private(const std::string &fname) {
    ...
    _is_stoped = false;
    _write_thread =
        std::thread(std::bind(&IOAdapter::Private::write_thread_func, this));
  }
  ~Private() { stop(); }
  ...
  void _append(const Chunk_Ptr &ch, Time min, Time max) {
    ...
    do {
      ...
      sqlite3_bind_blob(pStmt, 5, ch->header, sizeof(ChunkHeader),
                        SQLITE_STATIC);
  ...
  ChunksList readInterval(uint64_t period_from, uint64_t period_to,
                          Id meas_id) {
    ChunksList result;
    AsyncTask at = [this, period_from, period_to, meas_id,
                    &result](const ThreadInfo &ti) {
      TKIND_CHECK(THREAD_KINDS::DISK_IO, ti.kind);

      const std::string sql_query =
          "SELECT chunk_header, chunk_buffer FROM Chunks WHERE "
          ...
          if (rc == SQLITE_ROW) {
            ...
            if (!cptr->checkChecksum()) {
              logger_fatal("engine: io_adapter -  bad checksum of chunk #",
                           cptr->header->id, " for measurement id:",
                           cptr->header->first.id);
              continue;
            }
    ...
    lock();
    auto a = ThreadManager::instance()->post(THREAD_KINDS::DISK_IO, AT(at));
    a->wait();
  ...
  Chunk_Ptr readTimePoint(uint64_t period, Id meas_id) {
    lock();
    Chunk_Ptr result;
    AsyncTask at = [this, period, meas_id, &result](const ThreadInfo &ti) {
      TKIND_CHECK(THREAD_KINDS::DISK_IO, ti.kind);
      const std::string sql_query =
          "SELECT chunk_header, chunk_buffer FROM Chunks WHERE "
          "meas_id=? AND number==?";
    ...
    if (result == nullptr) {
      for (auto &cmm : _chunks_list) {
        if (cmm.ch->header->id == period &&
            cmm.ch->header->first.id == meas_id) {
          result = cmm.ch;
          break;
        }
      }
  ...
  Id2Meas currentValue() {
    Id2Meas result;
    AsyncTask at = [this, &result](const ThreadInfo &ti) {
      TKIND_CHECK(THREAD_KINDS::DISK_IO, ti.kind);
      const std::string sql_query =
          "SELECT a.chunk_header, a.chunk_buffer FROM "
  ...
  void replace(const Chunk_Ptr &ch, Time min, Time max) {
    AsyncTask at = [this, &ch, min, max](const ThreadInfo &ti) {
      TKIND_CHECK(THREAD_KINDS::DISK_IO, ti.kind);
      logger("engine: io_adapter - replace chunk #", ch->header->id, " id:",
             ch->header->first.id);
      const std::string sql_query =
          "UPDATE Chunks SET min_time=?, max_time=?, "
      ...
      do {
        ...
        sqlite3_bind_blob(pStmt, 3, ch->header, sizeof(ChunkHeader),
                          SQLITE_STATIC);
        sqlite3_bind_blob(pStmt, 4, ch->_buffer_t, ch->header->size,
                          SQLITE_STATIC);
  ...
  std::tuple<Time, Time> minMax() {
    Time min = MAX_TIME;
    Time max = MIN_TIME;
    AsyncTask at = [this, &min, &max](const ThreadInfo &ti) {
      TKIND_CHECK(THREAD_KINDS::DISK_IO, ti.kind);
  ...
  bool minMaxTime(Id id, Time *minResult, Time *maxResult) {
    bool result = false;
    AsyncTask at = [this, id, minResult, maxResult,
                    &result](const ThreadInfo &ti) {
      TKIND_CHECK(THREAD_KINDS::DISK_IO, ti.kind);
      logger("engine: io_adapter - minMaxTime #", id);
      const std::string sql_query =
          "SELECT min(min_time), max(max_time), "
          "count(max_time) FROM Chunks WHERE meas_id=?";
      sqlite3_stmt *pStmt;
      int rc;
      *minResult = MAX_TIME;
      *maxResult = MIN_TIME;
  ...
  void write_thread_func() {
    while (!_stop_flag) {
      ...
      std::list<ChunkMinMax> local_copy{_chunks_list.begin(),
                                        _chunks_list.end()};
      ...
      if (!local_copy.empty()) {
        ...
        logger("engine: io_adapter - dropping start. ", local_copy.size(),
               " chunks.");
        AsyncTask at = [this, &local_copy](const ThreadInfo &ti) {
          TKIND_CHECK(THREAD_KINDS::DISK_IO, ti.kind);

          sqlite3_exec(this->_db, "BEGIN TRANSACTION;", NULL, NULL, NULL);
...
IOAdapter::IOAdapter(const std::string &fname)
    : _impl(new IOAdapter::Private(fname)) {}

IOAdapter::~IOAdapter() { _impl = nullptr; }

void IOAdapter::stop() { _impl->stop(); }

void IOAdapter::append(const Chunk_Ptr &ch, Time min, Time max) {
  _impl->append(ch, min, max);
}

ChunksList IOAdapter::readInterval(uint64_t period_from, uint64_t period_to,
                                   Id meas_id) {
...
Id2Meas IOAdapter::currentValue() { return _impl->currentValue(); }

void IOAdapter::replace(const Chunk_Ptr &ch, Time min, Time max) {
  return _impl->replace(ch, min, max);
}

Time IOAdapter::minTime() { return _impl->minTime(); }

Time IOAdapter::maxTime() { return _impl->maxTime(); }

bool IOAdapter::minMaxTime(Id id, Time *minResult, Time *maxResult) {
  return _impl->minMaxTime(id, minResult, maxResult);
}

void IOAdapter::flush() { _impl->flush(); }

bystep::Description IOAdapter::description() { return _impl->description(); }
