...
public:
	ByStepClbk(dariadb::storage::ReaderClb*clb, dariadb::IdArray ids, dariadb::Time from, dariadb::Time to, dariadb::Time step) {
		_out_clbk = clb;
		_step = step;
        _from=from;
        _to=to;
...
void  Reader::readByStep(Meas::MeasList *output, dariadb::Time from, dariadb::Time to, dariadb::Time step) {
	std::unique_ptr<InnerCallback> clb(new InnerCallback(output));
